...
foreach($searchString as $string){
    ...
    elseif (!(strpos($string,'pc') === false) || !(strpos($string,'bo') === false)){
    ...
    elseif(!(strpos($string,'co') === false)){
        if (!strpos($query, 'AND (Brand="COCHAGEAR" OR Brand="CORSAIR")') ) $query = $query . ' AND (Brand="COCHAGEAR" OR Brand="CORSAIR")';
        $foundItem = True;
    }
    ...
    elseif (!(strpos($string,'raz') === false)){
        ...
        while($data = mysqli_fetch_assoc($rawData)){
            if (!(strpos(strtolower($data['Tags']), $string) === false)){
                if (!$start){
...
<body style="background-color: #ececec">
    ...
    <div class="max-w-screen-2xl mx-auto" style='font-family: Victor Mono'>
        <div class="flex flex-row justify-center">
            ...
            <div class="max-w-5xl 2xl:max-w-6xl min-w-[50%]">
