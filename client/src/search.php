...
if (isset($_GET['search'])) $searchString = explode('+', str_replace('-','',strtolower(urlencode(_vn2en($_GET['search'])))));
else $searchString = [];
$query = 'SELECT * FROM products WHERE 1=1 ';
$foundItem = False;
foreach($searchString as $string){
    if (!(strpos($string,'man') === false) || !(strpos($string,'hinh') === false)|| !(strpos($string,'screen') === false)){
        if (!strpos($query, 'AND Category="Screen"')) $query = $query . ' AND Category="Screen" ';
        $foundItem = True;
    }
    elseif (!(strpos($string,'ban') === false)|| !(strpos($string,'phim') === false) || !(strpos($string,'keyboard') === false)){
        if (!strpos($query, 'AND Category="Keyboard"')) $query = $query . ' AND Category="Keyboard" ';
        $foundItem = True;
    }
    elseif (!(strpos($string,'tai') === false) || !(strpos($string,'nghe') === false) || !(strpos($string,'headphone') === false) || !(strpos($string,'earphone') === false)){
        if (!strpos($query, 'AND Category="Headphone"')) $query = $query . ' AND Category="Headphone" ';
        $foundItem = True;
    }
    elseif (!(strpos($string,'chuot') === false) || !(strpos($string,'mouse') === false)){
        if (!strpos($query, 'AND Category="Mouse"')) $query = $query . ' AND Category="Mouse" ';
        $foundItem = True;
    }
    elseif (!(strpos($string,'pc') === false) || !(strpos($string,'may bo') === false)){
        if (!strpos($query, 'AND Category="PC"')) $query = $query . ' AND Category="PC" ';
        $foundItem = True;
    }
    elseif (!(strpos($string,'tay') === false) || !(strpos($string,'cam') === false) || !(strpos($string,'lap') === false) || !(strpos($string,'top') === false)){
        if (!strpos($query, 'AND Category="Laptop"')) $query = $query . ' AND Category="Laptop" ';
        $foundItem = True;
    }
    elseif (!(strpos($string,'may') === false) || !(strpos($string,'tinh') === false)){
        if (!strpos($query, 'AND (Category="PC" OR Category="Laptop")') ) $query = $query . ' AND (Category="PC" OR Category="Laptop")';
        $foundItem = True;
    }
    elseif (!(strpos($string,'asus') === false)){
        if (!strpos($query, 'AND Brand="ASUS"') ) $query = $query . ' AND Brand="ASUS"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'acer') === false)){
        if (!strpos($query, 'AND Brand="ACER"') ) $query = $query . ' AND Brand="ACER"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'cor') === false)){
        if (!strpos($query, 'AND Brand="CORSAIR"') ) $query = $query . ' AND Brand="CORSAIR"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'gear') === false) || !(strpos($string,'ccg') === false) || !(strpos($string,'cha') === false)){
        if (!strpos($query, 'AND Brand="COCHAGEAR"') ) $query = $query . ' AND Brand="COCHAGEAR"';
        $foundItem = True;
    }
    elseif(!(strpos($string,'co') === false)){
        if (!strpos($query, 'AND (Brand="COCHAGEAR" OR Brand="CORSAIR" OR Category="Keyboard" OR Category="Headphone" OR Category="Mouse")') ) $query = $query . ' AND (Brand="COCHAGEAR" OR Brand="CORSAIR" OR Category="Keyboard" OR Category="Headphone" OR Category="Mouse")';
        $foundItem = True;
    }
    elseif (!(strpos($string,'dare') === false)){
        if (!strpos($query, 'AND Brand="DAREU"') ) $query = $query . ' AND Brand="DAREU"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'del') === false)){
        if (!strpos($query, 'AND Brand="DELL"') ) $query = $query . ' AND Brand="DELL"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'dra') === false)){
        if (!strpos($query, 'AND Brand="E-DRA"') ) $query = $query . ' AND Brand="E-DRA"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'edi') === false)){
        if (!strpos($query, 'AND Brand="EDIFIER"') ) $query = $query . ' AND Brand="EDIFIER"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'giga') === false) || !(strpos($string,'byte') === false)){
        if (!strpos($query, 'AND Brand="GIGABYTE"') ) $query = $query . ' AND Brand="GIGABYTE"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'log') === false) || !(strpos($string,'tec') === false)){
        if (!strpos($query, 'AND Brand="LOGITECH"') ) $query = $query . ' AND Brand="LOGITECH"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'ms') === false)){
        if (!strpos($query, 'AND Brand="MSI"') ) $query = $query . ' AND Brand="MSI"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'rap') === false)){
        if (!strpos($query, 'AND Brand="RAPOO"') ) $query = $query . ' AND Brand="RAPOO"';
        $foundItem = True;
    }
    elseif (!(strpos($string,'raz') === false)){
        if (!strpos($query, 'AND Brand="RAZER"') ) $query = $query . ' AND Brand="RAZER"';
        $foundItem = True;
    }
    else{
        $tmpQuery = 'SELECT * FROM products';
        $rawData = mysqli_query($con, $tmpQuery);
        $start = False;
        $query .= ' AND (ID_Product="';
        while($data = mysqli_fetch_assoc($rawData)){
            if (!(strpos(str_replace('-','',_vn2en(strtolower($data['Tags']))), strval($string)) === false) || !(strpos(_vn2en(strtolower($data['Name'])), strval($string)) === false)){
                if (!$start){
                    if (!strpos($query, 'AND ID_Product='. $data["ID_Product"])) $query .= $data["ID_Product"] . '"';
                    $foundItem = True;
                    $start = True;
                }
                else{
                    if (!strpos($query, 'AND ID_Product='. $data["ID_Product"])) $query = $query . ' OR ID_Product="'. $data["ID_Product"].'"';
                    $foundItem = True;
                }
            }
        }
...
$returnedList = [];
$res = '';
while($rawData && $data = mysqli_fetch_assoc($rawData)){
    ...
    switch ($data['Category']) {
        case 'Keyboard':
            ...
            $res = '<a href="'. '../src/item.php/'.$data['slug']. '" class="border p-3 rounded-md hover:shadow-lg transition-shadow duration-200 ease-in-out cursor-pointer">
            <img class="w-full h-40 object-cover mb-5" src="' . $Image['Image'] . '" alt="' . $data['Name'] . '">
            <div class="text-sm font-bold mb-5 h-24">' . $data['Name'] . '</div>
...
<body style="background-color: #ececec">
    ...
    <div class="max-w-screen-2xl mx-auto" style='font-family: Victor Mono'>
        <div class="flex flex-row justify-center">
            ...
            <div class="max-w-5xl 2xl:max-w-6xl min-w-[70%]">
                <div class="rounded-lg bg-white w-full shadow-md p-4 mt-10 mb-10">
                    <div class="font-bold text-xl mb-5">| KẾT QUẢ TÌM KIẾM</div>
                    <div class="mb-5">Hiển thị <span class="font-bold"><?php echo sizeof($returnedList)?>
                        </span> sản phẩm liên quan đến <span class="font-bold">
                            <?php
                                if(isset($_GET['search'])) echo str_replace('+', " ",_vn2en($_GET['search'])); else echo "";
                            ?>
                        </span>
                    </div>
                    <div class="grid grid-cols-5 gap-4">
                        <?php
                            for($i = 0; $i < sizeof($returnedList); $i++) {
                                echo $returnedList[$i];
                            }
                        ?>
                    </div>
