    ...
    if(isset($_POST)){
        try{
            ...
            if(isset($name) && isset($email) && isset($phone_number) && isset($hashed_password) && empty(mysqli_fetch_assoc($result))){
                ...
                $query = "INSERT INTO members (Name, Phone_Number, Email, Account_Password, Image, Role, Hash_ID, Gender, Address_1, Address_2, Address_3, Status) 
                VALUES ('$name', '$phone_number', '$email', '$hashed_password','images/logoBK.png','user', '$hash_id', '', '','','' , 'active')"; 
